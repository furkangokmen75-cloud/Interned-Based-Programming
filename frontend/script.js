...
function renderAuthArea() {
  ...
  if (currentUser) {
    area.innerHTML = `
      ...
      ${currentUser.IsAdmin == 1 ? '<span class="tag" style="color:var(--accent);border-color:var(--accent);">Admin</span>' : ''}
...
function scrollToAbout(e) {
  ...
  document.querySelectorAll('.top-nav-link[href="#about"]').forEach(l => l.classList.add('active'));
}

function scrollToTop() {
  window.scrollTo({ top: 0, behavior: 'smooth' });
  document.querySelectorAll('.top-nav-link').forEach(l => l.classList.remove('active'));
  document.querySelector('.top-nav-link[href="#explore"]')?.classList.add('active');
}
...
window.addEventListener('scroll', () => {
  ...
  sections.forEach(id => {
    ...
    if (!el) return;
    const link = document.querySelector(`.top-nav-link[href="#${id}"]`);
    if (!link) return;
